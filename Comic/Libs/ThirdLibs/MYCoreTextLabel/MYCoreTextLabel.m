...
- (MYLinkModel *)selectedLink:(CGPoint)touchPoint
{
    ...
    for (MYLinkModel *link in self.links) {
        ...
        for (UITextSelectionRect *rect in link.rects) {
            ...
            if (CGRectContainsPoint(rect.rect, touchPoint)) {
                ...
                //回调内容
                if ([self.delegate respondsToSelector:@selector(linkText:type:tag:)]) {
                    [self.delegate linkText:link.linkText type:link.linkType tag:self.tag];
                }
                break;
            }
        }
    ...
    if (isBool==NO) {
        if ([self.delegate respondsToSelector:@selector(linkMoretag:)]) {
            [self.delegate linkMoretag:self.tag];
        }
    }
    return linkModel;
}
